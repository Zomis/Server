...
public class BattleshipGame extends Game {
    ...
    protected boolean makeMove(Command command, int player) {
        if (command.getParameter(1).equals("SHIP")) {
        ...
        if (command.getParameter(1).equals("MOVE")) {
            if (currentPlayer != player) {
                this.send("ERRR Player " + player + " is not in turn to play. Disqualified");
                this.endGame();
            } else {
                boolean hit = makeMove(player, command.getParameterInt(2), command.getParameterInt(3));
    ...
    private boolean placeShips(int player, Command command) {
        ...
        while (it.hasNext()) {
            ...
            if (!correctFlipped && !correctUnflipped) {
                this.send("ERRR Player " + player + " sent incorrect dimensions of ship: " + ship);
                this.endGame();
                return false;
            }
            Battleship yourShip = ship.atPos(correctFlipped, x, y);
            if (!yourShip.inRange(mapWidth, mapHeight)) {
                this.send("ERRR Ship " + ship + " does not fit inside range: " + mapWidth + "x" + mapHeight);
                this.endGame();
                return false;
            }

            if (yourShip.collidesWith(yourShips)) {
                this.send("ERRR Ship " + ship + " does not fit inside range: " + mapWidth + "x" + mapHeight);
                this.endGame();
                return false;
            }
